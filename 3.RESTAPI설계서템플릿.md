...
| **204** | No Content | 성공 (응답 데이터 없음) | 로그아웃 성공 |
...
| 역할 | 권한 | 설명 |
|------|------|------|
| **USER** | 일반 사용자 | 본인 프로젝트/업무 조회, 본인 Task 상태 변경, 연차 신청/조회 |
| **ADMIN** | 관리자 | 프로젝트 생성, Task 생성/담당자 배정, 연차 승인/반려 |
...
Content-Type: application/json
Authorization: Bearer {JWT_TOKEN}
Required Role: USER(본인 Task), ADMIN
...
Response (403 Forbidden):
{
  ...
  "error": {
    ...
    "message": "본인 Task만 상태 변경할 수 있습니다"
  }
}
```
...
| **TASK_ACCESS_DENIED** | 본인 Task가 아님 | 담당자 권한 확인 |
...
- 캘린더 페이지: `GET /calendar/events`, `GET /attendance/summary`
...
- USER: 본인 프로젝트/본인 Task/본인 연차 API만 허용
- ADMIN: 전체 프로젝트/Task 관리, Task 담당자 배정, 전체 연차 신청 조회 및 승인·반려 API 허용
...
### 9.4 확정 필요 항목 API 표기
- 실제 출퇴근 기록 기능 확정 전까지 `check-in/check-out` 엔드포인트는 `Optional`로 문서화
- 확정 시 `POST /attendance/check-in`, `POST /attendance/check-out` 활성화
